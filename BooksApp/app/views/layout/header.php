        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                ...
                <div class="pt-6 mt-6 border-t border-slate-800">
                    ...
                    <div class="px-4 text-teal-400 font-medium mb-4 flex items-center justify-between gap-2">
                        <span class="truncate"><?= htmlspecialchars($_SESSION['user_name'] ?? 'Uživatel') ?></span>
                        <?php if (isset($_SESSION['is_admin']) && $_SESSION['is_admin'] == 1): ?>
                            <span class="bg-teal-500/20 text-teal-400 text-[10px] font-bold px-2 py-0.5 rounded border border-teal-500/30 uppercase tracking-wider shrink-0">Admin</span>
                        <?php endif; ?>
                    </div>
                ...
                <div class="space-y-3" id="flash-messages">
                    <?php foreach ($_SESSION['messages'] as $type => $messages): ?>
                        ...
                        <?php foreach ($messages as $message): ?>
                            <div class="<?= $style ?> border-l-4 p-4 rounded-r-lg shadow-sm flex justify-between items-center flash-msg transition-opacity duration-500">
                                <p class="font-medium text-sm"><?= htmlspecialchars($message) ?></p>
                                <button onclick="this.parentElement.style.display='none'" class="text-current opacity-50 hover:opacity-100 text-lg leading-none p-1 font-bold">&times;</button>
                            </div>
                        <?php endforeach; ?>
                    <?php endforeach; ?>
