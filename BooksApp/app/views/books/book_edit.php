    ...
    <form action="<?= BASE_URL ?>/index.php?url=book/update/<?= htmlspecialchars($book['id']) ?>" method="post" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" id="title" name="title" value="<?= htmlspecialchars($book['title']) ?>" required class="w-full bg-slate-50/50 border-0 text-slate-900 rounded-xl px-4 py-3.5 ring-1 ring-inset ring-slate-200 focus:ring-2 focus:ring-teal-500 focus:bg-white transition-all">
            </div>

            <div class="sm:col-span-2">
                <label for="author" class="block text-sm font-semibold text-slate-600 mb-2">Autor / Vývojář <span class="text-teal-500">*</span></label>
                <input type="text" id="author" name="author" value="<?= htmlspecialchars($book['author']) ?>" required class="w-full bg-slate-50/50 border-0 text-slate-900 rounded-xl px-4 py-3.5 ring-1 ring-inset ring-slate-200 focus:ring-2 focus:ring-teal-500 focus:bg-white transition-all">
            </div>
            ...
            <!-- ZMĚNA: Kategorie s předvýběrem (selected) -->
            <div>
            ...
            <!-- ZMĚNA: Subkategorie s předvýběrem (selected) -->
            <div>
                ...
                <select id="subcategory" name="subcategory" class="w-full bg-slate-50/50 border-0 rounded-xl px-4 py-3.5 ring-1 ring-inset ring-slate-200 focus:ring-2 focus:ring-teal-500 transition-all">
                    ...
                    <?php foreach ($subcategories as $sub): ?>
                        <?php $isSelected = ($book['subcategory'] == $sub['id']) ? 'selected' : ''; ?>
